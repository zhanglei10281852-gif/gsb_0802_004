...
import { computeBlockers, isReady, latestEvidencePerConsumer, summarizeEvidence } from '../core/gate.js';
...
export type EvidenceResult =
  | { accepted: true; evidence: EvidenceRecord; deduped: boolean; proposal: StoredProposal }
...
function rowToProposal(row: ProposalRow): StoredProposal {
  const decision = row.decision_json ? (JSON.parse(row.decision_json) as DecisionSnapshot) : null;
  return {
...
export class ProposalRepository {
  ...
  create(input: ProposalInput): { proposal: StoredProposal; event: CausalEvent } {
    ...
    const compatibility = checkCompatibility(input.baseline, input.candidate, this.clock);
    ...
    const tx = this.db.transaction((): { proposal: StoredProposal; event: CausalEvent } => {
      this.db
        .prepare(
  ...
  getEvidence(proposalId: ProposalId): EvidenceRecord[] {
    const rows = this.db
      .prepare('SELECT * FROM evidence WHERE proposal_id = ? ORDER BY received_at ASC, evidence_id ASC')
  ...
  ingestEvidence(input: EvidenceInput): EvidenceResult {
    ...
    const known = proposal.consumers.some((c) => c.consumerId === input.consumerId);
    if (!known) {
      this.append(input.proposalId, now, 'evidence-rejected', {
    ...
    const existing = this.db
      .prepare('SELECT * FROM evidence WHERE proposal_id = ? AND idempotency_key = ?')
      .get(input.proposalId, input.idempotencyKey) as EvidenceRow | undefined;

    if (existing) {
      return { accepted: true, deduped: true, evidence: rowToEvidence(existing), proposal };
    }
    ...
    try {
      ...
      return { accepted: true, deduped: false, evidence: rowToEvidence(row), proposal };
    } catch (err) {
      ...
      if (message.includes('UNIQUE') && message.includes('idempotency_key')) {
        ...
        return { accepted: true, deduped: true, evidence: rowToEvidence(row), proposal };
      }
      throw err;
    }
  }

  refreshGateStatus(proposalId: ProposalId): { proposal: StoredProposal; changed: boolean; blockers: ReturnType<typeof computeBlockers> } {
    ...
    if (blockers.length === 0) {
      nextStatus = proposal.status === 'open' || proposal.status === 'collecting' ? 'ready' : proposal.status;
    ...
    if (nextStatus !== proposal.status) {
      ...
      return { proposal: this.requireById(proposalId), changed: true, blockers };
    }
    return { proposal, changed: false, blockers };
  }

  decide(input: DecisionInput): { proposal: StoredProposal; event: CausalEvent; blockers: ReturnType<typeof computeBlockers> } {
    const now = this.clock.now();
    const proposal = this.requireById(input.proposalId);
    ...
    const finalStatus: StoredProposal['status'] = input.kind === 'approve' ? 'approved' : 'rejected';

    const result = this.db.transaction((): { proposal: StoredProposal; event: CausalEvent; blockers: ReturnType<typeof computeBlockers> } => {
      const res = this.db
        .prepare(
          `UPDATE proposals
      ...
      if (res.changes === 0) {
        throw new ConflictError('concurrent decision: proposal was already decided by another request');
      }
      ...
      return { proposal: this.requireById(input.proposalId), event, blockers };
    })();
